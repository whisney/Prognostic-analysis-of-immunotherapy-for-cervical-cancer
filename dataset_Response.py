...
class Dataset_ST(Dataset):
    def __init__(self, data_dir, split_path, metadata_path, data_set='train', augment=True):
    ...
    def __getitem__(self, idx):
        ID = self.ID_list[idx]
        label = int(self.metadata_df.loc[self.metadata_df.ID == ID, 'Label'].values[0])
        ...
        return imgs, modality_sign, Clinical_features, label
